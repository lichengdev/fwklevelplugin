...
public class JPEGHuffmanTable
{
     ...
     * @exception IllegalArgumentException if the length of <code>lengths</code> is greater than 17
     * @exception IllegalArgumentException if the length of <code>symbols</code> is greater than 256
     * @exception IllegalArgumentException if any of the values in <code>lengths</code> or
     *                <code>symbols</code> is less than zero
     */
    public JPEGHuffmanTable(short lengths[], short symbols[])
    {
        if (lengths.length > HUFF_MAX_LEN) throw new IllegalArgumentException("lengths array is too long");
        for (int i = 1; i < lengths.length; i++)
            if (lengths[i] < 0) throw new IllegalArgumentException("Values in lengths array must be non-negative.");
        
        if (symbols.length > HUFF_MAX_SYM) throw new IllegalArgumentException("symbols array is too long");
        for (int i = 0; i < symbols.length; i++)
            if (symbols[i] < 0) throw new IllegalArgumentException("Values in symbols array must be non-negative.");
    ...
    /**
     * This checks that the table they gave us isn't 'illegal' It checks that the symbol length counts are
     * possible, and that they gave us at least enough symbols for the symbol length counts. Eventually this
     * might check that there aren't duplicate symbols.
     */
    private void checkTable()
    {
        ...
        if (numVals < 0) throw new IllegalArgumentException("Invalid Huffman Table provided, lengths are incorrect.");
        
        // I'll let them go if they gave us 'extra' symbols...
        if (sum > symbols.length) throw new IllegalArgumentException("Invalid Huffman Table provided, not enough symbols.");
    }
    
    /**
     * Return a copy of the array containing the number of symbols for each length in the Huffman table.
     * @return A short array where array[k] = # of symbols in the table of length k. array[0] is unused
     */
    ...
    /**
     * Return an array containing the Huffman symbols arranged by increasing length. To make use of this array
     * you must refer the the lengths array.
